...
import test from 'ava';
  ...
  var testType = (ds, t, elen) => test('Type:' + t, assert => {
    ...
    assert.is(ds.position , elen*i);
    assert.is(ds.byteLength , blen);
    assert.is(ds.buffer.byteLength , ds.byteLength+boff);
    ds.seek(0);
    for (i=0; i<Math.floor(ds.byteLength/elen); i++) {
      assert.is(ds["read"+t](), 125);
    }
    assert.is(ds.position , elen*i);
    assert.is(ds.byteLength , blen);
    assert.is(ds.buffer.byteLength , ds.byteLength+boff);
  ...
  var testSubArray = (typedArrayConstructor, t, arr) => test('SubArray:' + t, assert => {
      ...
      assert.deepEqual(typedArr.subarray(1), outSubArray);
  });
  ...
  var testDS = (i, ds, boff, blen, t, elen, arr) => test('DS:' + t, assert => {
    ...
    for (i=0; i<Math.floor(ds.byteLength/elen); i++) {
      assert.is(rarr[i], arr[i]);
      assert.is(rarr[i], rarr2[i]);
    }
    assert.is(ds.position , elen*i);
    assert.is(ds.byteLength , blen);
    assert.is(ds.buffer.byteLength , ds.byteLength+boff);
  ...
  test('constructor', assert => {
    ...
    assert.is(ds.byteLength, buf.byteLength);
    assert.is(ds.endianness, DataStream.LITTLE_ENDIAN);
    for (var i=0; i<100; i++) {
      ds = new DataStream(buf, i);
      ds = new DataStream(buf, i, DataStream.BIG_ENDIAN);
      ds = new DataStream(buf, i, DataStream.LITTLE_ENDIAN);
      assert.is(ds.byteLength, buf.byteLength-i);
      assert.is(ds.byteOffset, i);
    }
    ds = new DataStream(buf, 2, DataStream.BIG_ENDIAN);
    assert.is(ds.endianness, DataStream.BIG_ENDIAN);
    ...
    assert.is(ds.endianness, DataStream.BIG_ENDIAN);
    assert.is(ds.byteLength, buf.byteLength);
    assert.is(ds.byteOffset, dv.byteOffset);
    for (var i=0; i<100; i++) {
    ...
    for (var i=0; i<50; i++) {
      ...
      assert.is(ds.byteLength, buf.byteLength-i-dv.byteOffset);
      assert.is(ds.byteOffset, dv.byteOffset+i);
    }
    for (var i=0; i<100; i++) {
    ...
    for (var i=0; i<25; i++) {
      ...
      ds = new DataStream(dv);
      assert.is(ds.byteLength, buf.byteLength-i*4);
      assert.is(ds.byteOffset, dv.byteOffset);
    }
    for (var i=0; i<12; i++) {
  ...
  test('Struct', assert => {
    ...
    delete obj.data;
    delete obj2.data;
    assert.is(255, obj.endNote);
    assert.is(255, obj2.endNote);
    assert.is(JSON.stringify(obj), JSON.stringify(obj2));
    ...
    delete obj3.data;
    ds.seek(p);
    assert.is(255, obj3.endNote);
    assert.deepEqual(u, ds.readUint8Array(u.length) as any);
    assert.is(JSON.stringify(obj), JSON.stringify(obj3));
    ...
    assert.is(o2.one, 1);
    assert.is(o2.four, 1);
    assert.not(o2.two, o2.three);
    assert.is(o2.one, o2.four);
    assert.is(o2.one, o2.three);
    assert.not(o2.one/*LE*/, o2.two/*BE*/);
    assert.is(o2.one/*LE*/, o2.three/*LE*/);
    ds2.seek(0);
    ds2.endianness = DataStream.BIG_ENDIAN;
    o2 = ds2.readStruct(def2);
    assert.not(o2.one, 1);
    assert.not(o2.four, 1);
    assert.not(o2.two, o2.three);
    assert.is(o2.one, o2.four);
    assert.is(o2.one/*BE*/, o2.two/*BE*/);
    assert.not(o2.one/*BE*/, o2.three/*LE*/);
    ...
    assert.is(o3.length, 8);
    assert.is(o3.endNote, 255);
    assert.deepEqual(o3.data, [1,2,3,4,5,6]);
    ...
    assert.is(o4.length, 8);
    assert.is(o4.endNote, 255);
    assert.deepEqual(o4.data.odd, [1,3,5]);
    assert.deepEqual(o4.data.even, [2,4,6]);
    var pos = ds4.position;
    ds4.writeStruct(def4, o4);
    ds4.seek(pos);
    var o4b = ds4.readStruct(def4);
    assert.is(JSON.stringify(o4), JSON.stringify(o4b));
    assert.deepEqual(u4.concat(u4), new Uint8Array(ds4.buffer) as any);
	...
	assert.is(o5.len, o5.greet.length);
    assert.is('Hello', o5.greet);
    assert.is(', ', o5.pad);
	assert.is(o5.len2, o5.greet2.length);
    assert.is('World!', o5.greet2);
    assert.deepEqual([0,0,0,0,0,0,0,255], o5.tail);
    ...
    var u6 = [48,
        120, 105, 110, 32, 99, 104, 195, 131, 194, 160, 111, 32, 195, 132, 226, 128, 152, 195, 161, 194, 187, 226, 128, 148, 195, 161, 194, 187, 194, 175, 195, 161, 194, 186, 194, 171, 195, 161, 194, 186, 194, 168, 195, 161, 194, 187, 197, 184
    ];
    var ds6 = new DataStream();
    ds6.writeUint8Array(u6);
    ds6.seek(0);
    var o6: any = ds6.readStruct(def6);
    assert.is(greet, o6.greet);
    ...
    assert.is(JSON.stringify(o6), JSON.stringify(o6b));
    assert.is(JSON.stringify(o6), JSON.stringify(o6c));
    assert.deepEqual(u6, new Uint8Array(ds6.buffer) as any);
    assert.deepEqual(u6, new Uint8Array(ds6b.buffer) as any);
    assert.deepEqual(u6, new Uint8Array(ds6c.buffer) as any);
  });
  ...
  assert.is(ds.byteLength+64, buf.byteLength);
  ds.endianness = DataStream.LITTLE_ENDIAN;
  ds.writeUint16(1);
  ds.seek(0);
  ...
  assert.is(ds.position+64 , buf.byteLength);
  assert.is(ds.byteLength+64 , buf.byteLength);
  assert.is(ds.buffer.byteLength , buf.byteLength);

  ds.seek(0);
  for (var i = 0; i < 1000 / 8; i++) {
    assert.is(0.125 , ds.readFloat64());
  }
  assert.is(ds.position+64 , buf.byteLength);
  ...
  ds.endianness = DataStream.BIG_ENDIAN;
  for (var i = 0; i < 1000 / 8; i++) {
    assert.not(0.125, ds.readFloat64());
  }
  ds.seek(0);
  for (var i = 0; i < 999; i++) {
  ...
  ds.readFloat32();
  assert.is(ds.position , 4000);
  assert.is(ds.byteLength , 3996);
  assert.is(ds.buffer.byteLength , 3996+64);
  ds.position = 3996;
  ...
  ds.seek(0);
  for (var i = 0; i < 999; i++) {
    assert.is(0.125 , ds.readFloat32());
  }
  assert.is(ds.position+64 , ds.buffer.byteLength);
  ...
  assert.is(ds.position , 4000);
  assert.is(ds.byteLength , 4000);
  assert.is(ds.buffer.byteLength , ds.position+64);
  ...
  dss.seek(0);
  assert.is(s, dss.readUCS2String(s.length));

  var s = "Exif\\000\\000";
  var dss = new DataStream();
  dss.writeString(s);
  dss.seek(0);
  assert.is(s, dss.readString(s.length));

  var dss = new DataStream();
  var s = "Hello, World!";
  dss.writeCString(s);
  assert.is(dss.byteLength, s.length + 1);
  dss.seek(0);
  assert.is(s, dss.readCString());
  var dp = dss.position;
  ...
  dss.writeCString(s,s.length); // no zero terminate
  dss.seek(dp);
  assert.is(s, dss.readCString(s.length));
  dss.buffer;
  assert.is(s, dss.readCString());
  ...
  dss.seek(0);
  assert.is(s, dss.readString(dss.byteLength, 'UTF-8'));
  // ugh, byte-counted UTF-8 strings :(

  var dss = new DataStream();
  var s = "Hello, me";
  dss.writeString(s, 'UTF-8');
  assert.not(bl, dss.byteLength);
  dss.seek(0);
  assert.is(s, dss.readString(dss.byteLength, 'UTF-8'));
});
